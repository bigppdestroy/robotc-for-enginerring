...
/* i was trying to make the while loop being while(SensorValue(buttonswitch) == 1) but it didnt work so i had to make it an if statement inside the while loop*/
	while(true)
	{
		if(SensorValue(buttonswitch) == 1)
		{
			if(SensorValue(linefollow) <= 600)
			{
				turnLEDOff(greenLED);
				setServo(SErvo, 127);
